...
export async function uploadMedia(
  ...
  if(media_type.includes('video')) {
    const videoDurationSeconds = await getVideoDurationInSeconds(absolutePathToFile)
    initParams['video_duration_ms'] = (videoDurationSeconds*1000).toString()
  }

  const uploadInitResponse = await uploadInit(initParams, auth)

  await uploadAppend(auth,{absolutePathToFile,media_id: uploadInitResponse.media_id_string})

  const finalizeResponse = await uploadFinalize(auth,uploadInitResponse.media_id_string)

  let uploaded = false;
  while (!uploaded) {
    await new Promise((res)=>setTimeout(()=>{res(1)},finalizeResponse.processing_info.check_after_secs*1000))
    const uploadStatusResponse = await uploadStatus(auth,uploadInitResponse.media_id_string)
...
async function uploadInit(initParams: UploadInitParams,auth: TwitterAuth): Promise<UploadInitResponse> {
...
async function uploadAppend(auth: TwitterAuth,{absolutePathToFile,media_id}: UploadAppendParams): Promise<void> {
  const queryParams = new URLSearchParams({
    command: 'APPEND',
    segment_index: '0',
    media_id
...
// Create a new FormData instance
  ...
  Object.entries(formData.getHeaders()).forEach(([key,val])=>headers.set(key,val))
...
async function uploadFinalize(auth: TwitterAuth,media_id:string): Promise<UploadFinalizeResponse> {
...
async function uploadStatus(auth: TwitterAuth,media_id:string): Promise<UploadStatusResponse> {
  const queryParams = new URLSearchParams({
    command: 'STATUS',
    media_id
  });
